...
public class Questao6 {
	...
	public static void main(String[] args) {
		...
		while (flag) {
			...
			switch (op) {
				case 1:cadastrarAluno(leitor, alunos);break;
	...
	static void cadastrarAluno(Scanner leitor, String aluno[]) {
		...
		for(int i=0;i<contador;i++) {
			if(alunos[i].equals(nome)) {
				...
				if(presencaPorcent>25.0 && presencaPorcent<=100.0)
					System.out.printf("%.2f presença regular.",presencaPorcent);
				else if(presencaPorcent>=0.0 && presencaPorcent<25.0)
					System.out.printf("%.2f reprovado por falta.",presencaPorcent);
			}
		}
	}
}
